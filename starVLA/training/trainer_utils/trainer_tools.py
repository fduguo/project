...
def build_param_lr_groups(model, cfg):
    ...
    for module_name, lr in lr_cfg.items():
        ...
        try:
            for attr in module_name.split("."):
                module = getattr(module, attr)
            # filter out parameters frozen either by config or by the module itself
    ...
    if not param_groups:
        raise ValueError("No trainable parameters found when building optimizer param groups.")
