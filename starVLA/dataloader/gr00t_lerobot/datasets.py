...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._include_flow_pair = False
        self._flow_frame_delta = 1
        self._include_future_image = False
        action_cfg = modality_configs.get("action") if modality_configs is not None else None
        default_future_step_k = len(action_cfg.delta_indices) if action_cfg is not None else 1
        self._future_step_k = default_future_step_k
        if self.data_cfg is not None:
            self._include_flow_pair = self.data_cfg.get("include_flow_pair", False) not in ["False", False]
            self._flow_frame_delta = int(self.data_cfg.get("flow_frame_delta", 1))
            self._include_future_image = self.data_cfg.get("include_future_image", False) not in ["False", False]
            self._future_step_k = int(self.data_cfg.get("future_step_k", default_future_step_k))
    ...
    def _pack_sample(self, data: dict, trajectory_id: int | None = None, base_index: int | None = None) -> dict:
        ...
        if trajectory_id is not None:
            ...
            sample["episode_id"] = f"{self.dataset_name}:{int(trajectory_id)}"
        if base_index is not None:
            sample["timestep"] = int(base_index)
            sample["is_first_step"] = int(base_index) == 0
        if self._include_flow_pair:
            if trajectory_id is None or base_index is None:
                raise ValueError("include_flow_pair=True requires trajectory_id and base_index.")
            sample["flow_image_pair"] = self.get_flow_image_pair(trajectory_id, base_index)
        if self._include_future_image:
            if trajectory_id is None or base_index is None:
                raise ValueError("include_future_image=True requires trajectory_id and base_index.")
            sample["future_image"] = self.get_future_image(trajectory_id, base_index)
            sample["future_valid"] = self.is_future_image_valid(trajectory_id, base_index)
    ...
    def get_flow_image_pair(self, trajectory_id: int, base_index: int) -> list[list[Image.Image]]:
        """Return per-view [image_t-k, image_t] pairs for the flow expert."""
        pairs = []
        prev_index = int(base_index) - self._flow_frame_delta
        for video_key in self.modality_keys["video"]:
            original_delta = self.delta_indices[video_key]
            self.delta_indices[video_key] = np.array([0])
            try:
                first = self.get_video(trajectory_id, video_key, prev_index)[0]
                second = self.get_video(trajectory_id, video_key, int(base_index))[0]
            finally:
                self.delta_indices[video_key] = original_delta
            first = Image.fromarray(first)
            second = Image.fromarray(second)
            pairs.append([first, second])
        return pairs

    def is_future_image_valid(self, trajectory_id: int, base_index: int) -> bool:
        """Return whether image_{t+k} is inside the current trajectory."""
        trajectory_index = self.get_trajectory_index(trajectory_id)
        future_index = int(base_index) + self._future_step_k
        return bool(future_index < self.trajectory_lengths[trajectory_index])

    def get_future_image(self, trajectory_id: int, base_index: int) -> list[Image.Image]:
        """Return per-view future images at t + future_step_k, clamped by get_video."""
        future_images = []
        future_index = int(base_index) + self._future_step_k
        for video_key in self.modality_keys["video"]:
            original_delta = self.delta_indices[video_key]
            self.delta_indices[video_key] = np.array([0])
            try:
                image = self.get_video(trajectory_id, video_key, future_index)[0]
            finally:
                self.delta_indices[video_key] = original_delta
            future_images.append(Image.fromarray(image).resize((224, 224)))
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._sequential_steps: list[tuple[LeRobotSingleDataset, int, int]] = []
    ...
    def set_epoch(self, epoch: int):
        ...
        """
        self.epoch = epoch
        if getattr(self, "_sequential_step_sampling", False):
            self._build_sequential_steps()

    def _build_sequential_steps(self) -> None:
        rng = np.random.default_rng(safe_hash((self.epoch, self.seed, "sequential_steps")))
        episode_order = []
        for dataset in self.datasets:
            for trajectory_index, trajectory_id in enumerate(dataset.trajectory_ids):
                ...
                if trajectory_length > 0:
                    episode_order.append((dataset, int(trajectory_id), trajectory_length))

        if self.mode == "train":
            rng.shuffle(episode_order)

        self._sequential_steps = []
        for dataset, trajectory_id, trajectory_length in episode_order:
            for base_index in range(trajectory_length):
                self._sequential_steps.append((dataset, trajectory_id, base_index))

    def _sample_sequential_step(self, index: int) -> tuple[LeRobotSingleDataset, int, int]:
        if not self._sequential_steps:
            raise RuntimeError("No sequential steps available for mixture dataset.")
        return self._sequential_steps[index % len(self._sequential_steps)]

    def sample_step(self, index: int) -> tuple[LeRobotSingleDataset, int, int]:
        """Sample a single step from the dataset."""
        if self._sequential_step_sampling:
            return self._sample_sequential_step(index)
    ...
    def __len__(self) -> int:
        ...
        """
        if getattr(self, "_sequential_step_sampling", False):
            return len(getattr(self, "_sequential_steps", []))
