...
def _load_fast_processor(pretrained_path):
    try:
    ...
    with open(os.path.join(local_dir, "processor_config.json"), "r", encoding="utf-8") as f:
        cfg = json.load(f)
...
class Fast_Action_Tokenizer(nn.Module):
    ...
    def __init__(self, fast_tokenizer_name="/mnt/nas/gezuhao/zhouyuchen/playground/Pretrained_models/fast"):
        super().__init__()
...
def get_action_model(config=None):
    ...
    if config is not None and hasattr(config, "framework"):
        action_cfg = config.framework.get("fast_action_model", None)
        if action_cfg is None:
            action_cfg = config.framework.get("action_model", None)
        if action_cfg is not None:
            fast_tokenizer_name = action_cfg.get("fast_tokenizer_name", fast_tokenizer_name)
