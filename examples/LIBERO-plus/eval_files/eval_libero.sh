...
# CKPT=/path/to/checkpoint.pt bash examples/LIBERO-plus/eval_files/eval_libero.sh
...
LIBERO_HOME="${LIBERO_HOME:-/mnt/nas/gezuhao/zhouyuchen/project/LIBERO-plus}"
LIBERO_Python="${LIBERO_Python:-/mnt/nas/gezuhao/zhouyuchen/miniconda3/envs/libero-plus/bin/python}"
...
output_dir="${OUTPUT_DIR:-${model_root}/eval_outputs/libero_plus}"
...
wandb_project="${WANDB_EVAL_PROJECT:-${model_name}_libero_plus}"
...
for task_suite_name in "${task_suites[@]}"; do
    ...
    (
        set -o pipefail
        CUDA_VISIBLE_DEVICES="${suite_eval_gpu}" "${LIBERO_Python}" ./examples/LIBERO-plus/eval_files/eval_libero.py \
...
echo "All LIBERO-plus evaluation tasks finished. Logs saved to ${LOG_DIR}"
