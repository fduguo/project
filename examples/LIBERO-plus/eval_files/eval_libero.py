...
def eval_libero(args: Args) -> None:
    ...
    for task_id in range(num_tasks_in_suite):
        ...
        logging.info(f"[TASK] {task_id + 1}/{num_tasks_in_suite}: {task_description}")
        logging.info(f"[DEBUG] Task {task_id} original prompt: '{str(task_description).strip()}'")
        logging.info(f"[DEBUG] Task {task_id} model prompt: '{model_prompt}'")
        ...
        for episode_idx in range(args.num_trials_per_task):
            ...
            # Reset environment
            client_model.reset(task_description=model_prompt)  # Reset the client connection
            env.reset()
